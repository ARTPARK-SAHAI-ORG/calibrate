...
description: "CLI commands for text-only simulations between two Text to Text models."
---

Run text-only automated conversations between two Text to Text models (agent + simulated user). Each simulation pairs every persona with every scenario.
...
## pense ttt simulations run

Run Text to Text simulations from a configuration file.

```bash
pense ttt simulations run -c <config_file> -o <output_dir> -m <model> -p <provider> -n <parallel>
```
...
```bash
pense ttt simulations run -c ./config.json -o ./out
```

**Run with parallel simulations:**

```bash
pense ttt simulations run -c ./config.json -o ./out -n 4
```

**Use specific model:**

```bash
pense ttt simulations run -c ./config.json -o ./out -m openai/gpt-4.1 -p openrouter
```
...
## pense ttt simulations leaderboard

Generate a leaderboard comparing multiple simulation runs.

```bash
pense ttt simulations leaderboard -o <output_dir> -s <save_dir>
```
...
```bash
pense ttt simulations leaderboard -o ./out -s ./leaderboard
```
