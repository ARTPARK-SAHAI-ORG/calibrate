...
| --- | --- | --- | --- |
| `-a, --annotator-ids` | stringArray | — | **Required.** Annotator IDs to assign, creating one labelling job for each annotator. Must be in your workspace |
| `-c, --comments-enabled` | true | `true` | When true, the labelling form lets the annotator leave a comment on each item |
...
| `--q` | payload.name | — | Case-insensitive substring filter on payload.name. Applies only when `select_all=true` |
| `-r, --reasoning-mode` | optional | `optional` | How the labelling form treats the reasoning box on each judgement. optional shows it, `required` shows it and asks the annotator to fill it in, `hidden` leaves it out (options: optional, required, hidden) |
