...
<Tip>
  To grade an argument against a free-text criteria with an LLM judge instead of
  matching it exactly — including sub-parameters nested inside an object — see
  [Judging a parameter against criteria](/core-concepts/text-to-text#judging-a-parameter-against-criteria).
</Tip>
