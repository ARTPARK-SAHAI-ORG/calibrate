...
The numbers describe a single judge call, so a test case graded by three evaluators has three separate sets. A judge model that reports no usage leaves the fields out, and only `latency_seconds` is recorded.
