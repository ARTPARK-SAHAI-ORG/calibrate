...
<Note>
  In `--eval-only` mode, no model runs: the supplied `history` is treated as the full transcript and judged exactly as given (nothing is appended or modified). This is the key difference between the two modes for conversation tests.
</Note>
