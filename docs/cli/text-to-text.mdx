...
For arguments whose value varies (a generated message, a free-text summary), exact matching is too strict. Instead of a literal value, give the argument a **criteria spec** and an LLM judge decides whether the produced value satisfies it. Each argument is matched independently — mix literal (exact) and judged arguments in the same tool call:
    ...
    "tool_calls": [
      {
        ...
        "arguments": {
          ...
          "message": {
            ...
            "judge_model": "openai/gpt-4.1" // optional override
          }
        }
      }
    ]
...
| ------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| `"exact"`     | `{"match_type": "exact", "value": <literal>}`                          | Exact match; use to match a value that itself looks like a spec dict.                 |
| `"llm_judge"` | `{"match_type": "llm_judge", "criteria": "...", "judge_model": "..."}` | An LLM judge checks the produced value against `criteria`. `judge_model` is optional. |

Specs work at **any depth** — an argument that is an object can have its own sub-parameters mixed between exact and judged. Each judged sub-parameter is graded independently, and failures are reported with a dotted path (`patient.note`):
