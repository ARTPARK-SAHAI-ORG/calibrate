...
```bash
pense llm tests run -c <config_file> -o <output_dir> -m <model> -p <provider>
```
...
```bash
pense llm tests run -c ./config.json -o ./out -m openai/gpt-4.1 -p openrouter
```

**Use OpenAI directly:**

```bash
pense llm tests run -c ./config.json -o ./out -m gpt-4.1 -p openai
```
...
```bash
pense llm tests leaderboard -o <output_dir> -s <save_dir>
```
