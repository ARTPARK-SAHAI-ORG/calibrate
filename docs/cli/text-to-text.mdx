...
<CardGroup cols={2}>
  <Card title="Calibrate agent" icon="robot" href="/cli/calibrate-agent">
    Define your agent completely within Calibrate
  </Card>
  <Card
    title="Agent connection"
    icon="link"
    href="/cli/agent-connections/overview"
  >
...
### What each judge call cost

Every LLM judge call records what it cost and how long it took, alongside the grade it produced. In `results.json`, each evaluator's entry carries:

| Field                 | Meaning                                                              |
| --------------------- | -------------------------------------------------------------------- |
| `cost_usd`            | What OpenRouter charged for that single judge call, in US dollars.     |
| `input_tokens`        | Tokens sent to the judge model.                                        |
| `output_tokens`       | Tokens the judge model produced.                                       |
| `cached_input_tokens` | Input tokens the model served from its cache, included in `input_tokens`. |
| `latency_seconds`     | Wall-clock time for the call.                                          |

Tool call parameters graded with `llm_judge` criteria record the same fields on each entry under `param_judgments`.

The numbers describe a single judge call, so a test case graded by three evaluators has three separate sets. Latency is always recorded; the cost and token fields are left out for a judge model that reports no usage.
