...
2. **Provider** — OpenRouter or OpenAI (determines model options)
3. **Model selection** — multi-select from a list of suggested models:
   - Use Space to toggle selection, Enter to confirm
   - Provider-specific options are shown (e.g., `openai/gpt-4.1` for OpenRouter)
