...
<CardGroup cols={2}>
  <Card title="Calibrate agent" icon="robot" href="/cli/calibrate-agent">
    Define your agent's `system_prompt` and `tools` in the config file and
    evaluate across models.
  </Card>
  <Card title="Agent connection" icon="link" href="/cli/agent-connections/overview">
    Connect your existing agent — a [chat agent](/cli/agent-connections/chat)
    over HTTP for LLM tests — and run tests against it directly.
  </Card>
</CardGroup>
