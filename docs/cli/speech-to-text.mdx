...
- `metrics.json` with aggregate `wer`, evaluator summaries, and a `cost` object. The cost object includes `provider`, `pricing_model`, `currency`, `billing_unit`, `total_seconds`, `total_minutes`, `price_per_minute_usd`, and `estimated_total_cost_usd`.
