...
- `metrics.json` with aggregate `wer`, evaluator summaries, and a `cost` object. The cost object includes `provider`, `pricing_model`, `currency`, `billing_unit`, `total_seconds`, `audio_minutes`, the native per-minute rate (`cost_per_minute_usd`), and `cost_usd`. Providers billed in another currency (e.g. Sarvam in INR) instead report `cost_per_minute_inr`, the native-currency total `cost_in_currency`, and the live USD→currency rate used `cost_per_usd`; `cost_usd` is always present, converted at the current exchange rate (fetched live, with a fallback). Cost is a proportional estimate from bundled rates; provider invoices may differ because of per-file rounding, minimums, tiers, or regional pricing.
