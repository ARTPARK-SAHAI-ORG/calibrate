...
With tool calls — include a `tool_calls` array, each item a `tool` name and `arguments` object. You may also include the tool's `output`:
...
`output` is optional — the result your tool returned (any JSON value). It's shown in the results view for review only and never affects whether a test passes.
