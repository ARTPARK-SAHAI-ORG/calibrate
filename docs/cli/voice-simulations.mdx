...
**Structured Output (Default):** Use `"type": "structured_output"` (or `"client"`) for tools that extract structured data.

**Webhook:** Use `"type": "webhook"` for tools that call external APIs. Webhook tools make actual HTTP calls during voice simulations:
