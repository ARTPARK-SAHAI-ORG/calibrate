...
- `metrics.json` with evaluator summaries, `ttfb` percentiles, and a `cost` object. Each provider is priced in its **native billing unit** — no unit conversion. Character-billed providers (most) report `billing_unit: "character"`, `total_characters`, and the per-million-character rate (`cost_per_million_chars_usd`); audio-billed providers (OpenAI, Gemini) report `billing_unit: "minute"`, `total_seconds`, `audio_minutes`, and the per-minute rate (`cost_per_minute_usd`), with the audio duration measured from the synthesized output. Providers billed in another currency (e.g. Sarvam in INR) report the native rate (`cost_per_million_chars_inr`), the native-currency total `cost_in_currency`, and the live USD→currency rate used `cost_per_usd`. `cost_usd` is always present (converted at the current exchange rate, fetched live with a fallback) and is the metric comparable across all providers.
- `leaderboard/tts_leaderboard.xlsx` with cost columns in the summary sheet: `total_characters`, `cost_per_million_chars_usd`, `audio_minutes`, `cost_per_minute_usd`, and `cost_usd` (only the columns relevant to each provider's billing unit are populated).
