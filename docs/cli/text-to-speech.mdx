...
- `leaderboard/tts_leaderboard.xlsx` with cost columns in the summary sheet: `cost_per_million_chars_currency`, `cost_per_minute_currency`, and `cost_usd` (only the columns relevant to each provider's billing unit are populated).
