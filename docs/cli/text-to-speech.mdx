...
```csv
id,text,arguments
row_1,hello world,"{""pronunciation"": {""dialect"": ""Indian English""}}"
```
...
| `evaluators[].system_prompt` | string | Full system prompt used for this evaluator's audio LLM judge call. Supports `{{variable}}` placeholders that are filled from the row's `arguments` column.                                                                                                                                                                                                                                                                                                                  |
...
The `arguments` column holds a JSON object keyed by evaluator name. Each value is an object of variable names to values:
...
With an evaluator whose `system_prompt` reads `Mark True only if the audio is pronounced in {{dialect}}.`, row_1 is judged against `Mark True only if the audio is pronounced in Indian English.`
