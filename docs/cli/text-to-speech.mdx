...
- `metrics.json` with evaluator summaries, `ttfb` percentiles, and a `cost` object. The cost object includes `provider`, `pricing_model`, `currency`, `billing_unit`, `total_characters`, and `cost_per_million_chars_usd`. TTS providers bill per input character, so cost is reported as the per-million-character rate; models billed on audio output (OpenAI, Gemini) use an equivalent per-character rate so the metric is comparable across providers.
- `leaderboard/tts_leaderboard.xlsx` with cost columns in the summary sheet: `total_characters` and `cost_per_million_chars_usd`.
