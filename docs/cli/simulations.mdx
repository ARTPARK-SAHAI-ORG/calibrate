...
- **External voice agent (WebSocket)** — for voice simulations, point calibrate at an existing pipecat agent's WebSocket endpoint via `agent_url`. Calibrate streams real audio to it instead of running its own agent. See [Agent Connections → Pipecat WebSocket agents](/cli/agent-connections/websocket).
...
<Note>
  ...
  **voice** simulations against an existing agent, use a WebSocket `agent_url`
  ...
  agents](/cli/agent-connections/websocket).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="External voice agent (WebSocket) example">
    For voice simulations against an existing pipecat agent, set a `ws(s)://` `agent_url` and omit `system_prompt`, `tools`, `stt`, `tts`, and `llm`. See [Agent Connections → Pipecat WebSocket agents](/cli/agent-connections/websocket) for the full reference and metric caveats.
