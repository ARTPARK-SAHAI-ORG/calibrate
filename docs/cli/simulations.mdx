...
- **External voice agent (WebSocket)** — for voice simulations, point calibrate at an existing pipecat agent's WebSocket endpoint via `agent_url`. Calibrate streams real audio to it instead of running its own agent. See [Voice Agent Connections](/core-concepts/voice-agent-connections).
...
<Note>
  ...
  **voice** simulations against an existing agent, use a WebSocket `agent_url`
  (over `ws(s)://`) — see [Voice Agent
  Connections](/core-concepts/voice-agent-connections).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="External voice agent (WebSocket) example">
    For voice simulations against an existing pipecat agent, set a `ws(s)://` `agent_url` and omit `system_prompt`, `tools`, `stt`, `tts`, and `llm`. See [Voice Agent Connections](/core-concepts/voice-agent-connections) for the full reference and metric caveats.
