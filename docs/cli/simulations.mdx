...
Each simulation's `evaluation_results.csv` carries these as columns next to `value` and `reasoning`, one row per evaluator. Latency is always recorded; the cost and token cells are empty for a judge model that reports no usage.
