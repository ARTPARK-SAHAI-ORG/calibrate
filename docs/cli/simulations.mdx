...
Every LLM judge call records what it cost and how long it took, alongside the score it produced:
...
Each simulation's `evaluation_results.csv` carries these as columns next to `value` and `reasoning`, one row per evaluator. The run-level `results.csv` repeats them as `<evaluator>_<field>` columns, one row per simulation. A judge model that reports no usage leaves the fields out, and only `latency_seconds` is recorded.
