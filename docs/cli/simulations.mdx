...
<AccordionGroup>
  ...
  <Accordion title="Chat agent example">
  ...
  <Accordion title="Outbound voice agent example">
...
1. **Simulation type** — select "Voice"
2. **Config file** — path to your config file (with a `ws(s)://` `agent_url`)
3. **Parallel count** — how many simulations to run simultaneously (default: `1`)
4. **Output directory** — where results will be saved (defaults to `./out`)
5. **API keys** — enter the API keys for the user simulator's STT/TTS providers and the LLM judge
...
where `config.json` sets a `ws(s)://` `agent_url` (see [Agent Connections → Outbound voice agent](/cli/agent-connections/outbound-voice)).
