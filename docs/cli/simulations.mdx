...
There are three ways to run simulations:
...
- **Agent connection (text)** — point calibrate at your existing agent's HTTP endpoint. The user simulator sends messages to it and evaluates the full conversation.
- **External voice agent (WebSocket)** — for voice simulations, point calibrate at an existing pipecat agent's WebSocket endpoint via `agent_url`. Calibrate streams real audio to it instead of running its own agent. See [Agent Connections → Pipecat WebSocket agents](/core-concepts/agent-connections#pipecat-websocket-agents-voice).

<Note>
  The HTTP agent connection (over `http(s)://`) is for **text** simulations. For
  **voice** simulations against an existing agent, use a WebSocket `agent_url`
  (over `ws(s)://`) — see [Agent Connections → Pipecat WebSocket
  agents](/core-concepts/agent-connections#pipecat-websocket-agents-voice).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="External voice agent (WebSocket) example">
    For voice simulations against an existing pipecat agent, set a `ws(s)://` `agent_url` and omit `system_prompt`, `tools`, `stt`, `tts`, and `llm`. See [Agent Connections → Pipecat WebSocket agents](/core-concepts/agent-connections#pipecat-websocket-agents-voice) for the full reference and metric caveats.
