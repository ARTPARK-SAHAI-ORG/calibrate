...
| `arguments`   | object | Optional. Keyed by evaluator name, it fills the `{{variable}}` placeholders in that evaluator's `system_prompt` for every simulation using this scenario |
  ...
  "scenarios": [
    ...
    },
    {
      ...
      "arguments": {
        "goal_met": {
          "order_id": "ORD-67890",
          "expected": "the agent asks for the order ID"
        }
      }
    }
  ]
...
| `system_prompt` | string  | Full system prompt for this evaluator's LLM judge call. Include any agent-context the judge needs to score correctly. Supports `{{variable}}` placeholders that are filled from the scenario's `arguments`. |
...
A simulation pairs one persona with one scenario. The scenario can pass `arguments` to an evaluator. Each `{{variable}}` placeholder in that evaluator's `system_prompt` is replaced with the matching value before the judge call, so one evaluator can be reused across scenarios that differ only in their details.

`arguments` is an object keyed by evaluator name. Each value is an object of variable names to values:

```json
{
  "name": "vague inquiry",
  "description": "Start by asking about your order without providing the order ID.",
  "arguments": {
    "goal_met": {
      "order_id": "ORD-67890",
      "expected": "the agent asks for the order ID"
...
With an evaluator named `goal_met` whose `system_prompt` reads `The customer's order is {{order_id}}. Mark True if {{expected}}.`, every simulation using this scenario is judged against `The customer's order is ORD-67890. Mark True if the agent asks for the order ID.`
...
<AccordionGroup>
  ...
  <Accordion title="Agent connection (Outbound voice agent) example">
    ...
    Refer to [Agent Connections → Outbound voice agent](/cli/agent-connections/outbound-voice) for the full reference.
