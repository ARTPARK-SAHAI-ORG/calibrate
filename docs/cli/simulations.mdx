...
<AccordionGroup>
  ...
  <Accordion title="Outbound voice agent example">
    For voice simulations against an existing pipecat agent, set a `ws(s)://` `agent_url` and omit `system_prompt`, `tools`, `stt`, `tts`, and `llm`. See [Agent Connections → Outbound voice agent](/cli/agent-connections/outbound-voice) for the full reference and metric caveats.
