...
- **`pipeline`** (the default) streams every clip through a real pipecat agent pipeline at real-time pace, using the same STT configuration your live voice agent deploys, following the methodology of the [pipecat STT benchmark](https://github.com/pipecat-ai/stt-benchmark). It reflects the accuracy and behavior an agent actually sees in production, and it also measures time-to-final-transcript (TTFS) latency. Because it runs in real time, it is slower.
