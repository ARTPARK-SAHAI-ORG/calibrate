...
**`results.csv`** has one row per clip:

```csv
id,gt,pred,wer,cer,audio_duration_seconds,ttfs,semantic_match,semantic_match_reasoning
audio_1,Hi,Hi,0.0,0.0,1.2,0.41,True,"Values match exactly."
audio_2,Madam my name is Geeta Shankar,madam my name is geetha shankar,0.14,0.06,3.4,0.55,True,"Spelling differs but the value is the same."
```

`ttfs` appears only on the `pipeline` engine. The Sarvam and semantic-WER judges add `sarvam_intent_score`, `sarvam_entity_score`, `sarvam_llm_wer`, `sarvam_llm_cer`, and `semantic_wer` columns, each with a matching `_reasoning` column.
...
Each configured evaluator (like `semantic_match`) appears as its own `{ "type", "mean" }` block, and `ttfs` only on the `pipeline` engine. Cost is a proportional estimate from bundled rates, so provider invoices may differ because of per-file rounding, minimums, tiers, or regional pricing.
...
| provider | wer | cer | semantic_match | semantic_wer | audio_minutes | cost_usd | count |
| --- | --- | --- | --- | --- | --- | --- | --- |
| openai | 0.00 | 0.00 | 1.00 | 0.00 | 2.14 | 0.013 | 3 |
| sarvam | 0.06 | 0.03 | 1.00 | 0.02 | 2.14 | 0.013 | 3 |
...
<AccordionGroup>
  <Accordion title="semantic_match (default evaluator system prompt)">
    Matches [`DEFAULT_STT_EVALUATOR`](https://github.com/ARTPARK-SAHAI-ORG/calibrate/blob/main/calibrate_agent/judges.py) in `calibrate_agent/judges.py` when no `--config` is passed.
...
These appear as extra columns (`sarvam_intent_score`, `sarvam_entity_score`, `sarvam_llm_wer`, `sarvam_llm_cer`, `semantic_wer`) in `metrics.json` and the leaderboard, alongside the `semantic_match` evaluator above.
