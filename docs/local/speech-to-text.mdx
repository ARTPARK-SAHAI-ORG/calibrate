...
Set the API keys for the selected providers in your environment (or a `.env` file in the directory you run the command from). The evaluation runs providers in parallel (max 2 at a time — tune with `--max-parallel`), showing the transcriptions as they are generated.

<Note>
  **How providers are transcribed — streaming pipeline vs direct.** Where a
  provider offers a real-time streaming API, Calibrate streams your audio
  through it — the same *pipeline* path a live voice agent uses — rather than
  only calling a batch whole-file endpoint. That reflects the accuracy an agent
  actually sees in production. Providers without a streaming API fall back to
  their direct (batch) transcription endpoint. This mirrors the methodology of
  the [pipecat STT benchmark](https://github.com/pipecat-ai/stt-benchmark).
</Note>
...
| `--max-parallel` | integer | `2` | Providers to evaluate concurrently (env `CALIBRATE_STT_MAX_PARALLEL`). |
