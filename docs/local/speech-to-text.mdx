...
`ttfs` appears only on the `pipeline` engine. Any evaluators you add with `--config` appear as their own `{ "type", "mean" }` block (e.g. `"semantic_match": { "type": "binary", "mean": 0.9 }`). Cost is a proportional estimate from bundled rates, so provider invoices may differ because of per-file rounding, minimums, tiers, or regional pricing.

**`leaderboard/stt_leaderboard.xlsx`** ranks every provider side by side (the same table printed to your terminal), one row per provider with WER/CER, each judge's mean, and the cost columns:

| provider | wer | cer | sarvam_entity_score | semantic_wer | audio_minutes | cost_usd | count |
| --- | --- | --- | --- | --- | --- | --- | --- |
| openai | 0.00 | 0.00 | 1.00 | 0.00 | 2.14 | 0.013 | 3 |
| sarvam | 0.06 | 0.03 | 0.95 | 0.02 | 2.14 | 0.013 | 3 |
