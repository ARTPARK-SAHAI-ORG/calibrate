...
- **`pipeline`** (the default) feeds each clip through a real pipecat pipeline at real-time pace, using the same STT setup your live voice agent runs, following the methodology of the [pipecat STT benchmark](https://github.com/pipecat-ai/stt-benchmark). The accuracy scores then match what your agent gets in production, and the run also measures latency: the gap between the speaker stopping and the final transcript arriving (TTFS). Running in real time makes it slower.
