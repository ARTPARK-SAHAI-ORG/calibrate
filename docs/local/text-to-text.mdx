...
<CardGroup cols={2}>
  <Card title="Calibrate agent" icon="robot" href="/local/calibrate-agent">
    Define your agent completely within Calibrate
  </Card>
  <Card
    title="Agent connection"
    icon="link"
    href="/local/agent-connections/overview"
  >
    ...
    "tool_calls": [
      {
        ...
        "arguments": {
          ...
          // judged by an LLM against the criteria
          "message": {
            "match_type": "llm_judge",
            "criteria": "A friendly appointment reminder that includes the date",
          },
        },
      },
    ],
...
A plain literal value (e.g. `"555-123-4567"`) is matched exactly, as usual. To opt an argument out of exact matching, give it a spec with one of these `match_type` values:
...
```jsonc
{
  ...
  "arguments": {
    "patient": {
      "name": "John Doe", // nested exact match
      "note": {
        // nested LLM-judged
        "match_type": "llm_judge",
        "criteria": "Describes a medical symptom or complaint",
      },
    },
  },
}
...
Like a [response test](#response-test), the agent is run on the `history` to produce its next reply — but instead of grading only that reply, the **entire resulting conversation** (history + the new reply) is graded by the simulation judge. Use this when correctness depends on the whole exchange, not just the latest turn. As with response tests, `history` ends at the user's turn that the agent should answer.

```jsonc
{
  // conversation test — agent replies, then the whole conversation is judged
  "id": "test-id-1",
  "history": [
    { "role": "assistant", "content": "Hello! How can I help you today?" },
    { "role": "user", "content": "I'd like to check order ORD-12345." },
  ],
  "evaluation": {
    "type": "conversation",
    "criteria": [
      { "name": "resolution", "arguments": { "topic": "order status" } },
      { "name": "tone" },
    ],
  },
}
  ...
  <Accordion title="Calibrate agent example">
    ...
    ```jsonc
    {
      ...
      "tools": [
        {
          ...
          "parameters": [
            ...
            },
            {
              "id": "summary",
              "type": "string",
              "description": "A short free-text summary of what the customer said",
              "required": true
            }
          ]
        },
      ...
      "test_cases": [
        ...
        {
          // tool_call — expect log_interaction (client tool). interaction_type
          // and sentiment are matched exactly; summary is free-text, so it is
          // graded by an LLM judge against a criteria instead of exact match.
          "id": "test-tool-call-2",
          "history": [
            {
              "role": "assistant",
            ...
            "tool_calls": [
              {
                ...
                "arguments": {
                  ...
                  "summary": {
                    "match_type": "llm_judge",
                    "criteria": "States that the customer is happy or satisfied with their recent purchase or its quality."
                  }
                }
              }
            ]
...
Pass your config with `-c` and the model(s) with `-m`. Set the API keys for the
selected provider in your environment (or a `.env` file in the directory you run
the command from). The evaluation runs the selected models in parallel (max 2 at
a time), showing results for each test as they complete.
...
```bash
calibrate-agent llm -c config.json -m openai/gpt-4.1 claude-sonnet-4-5 -p openrouter -o ./out
```
...
```bash
calibrate-agent llm -c config.json -o ./out
```
...
```bash
calibrate-agent llm -c config.json -m gemma-4-26b-a4b-it gpt-4o -o ./out
```
...
```bash
calibrate-agent llm -c config.json -o ./out --skip-verify
```

```bash
calibrate-agent llm -c config.json -m gemma-4-26b-a4b-it gpt-4o -o ./out --skip-verify
```
...
## Verify and evaluate existing outputs

Check that an external agent endpoint is reachable and returns the expected response format before running a full suite, with `--verify`:

```bash
calibrate-agent llm --verify --agent-url https://your-agent/chat
```

Or skip inference entirely and run the evaluators on outputs you already have, with `--eval-only`:

```bash
calibrate-agent llm --eval-only -c config.json --dataset outputs.json -o ./out
```

`--dataset` is a JSON list of `{test_case, output}` items. For agent-connection configs, `--skip-verify` runs without the pre-flight connection check.

## Options

| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `-c, --config` | string | — | Test config JSON (system prompt + tools + test cases, or an `agent_url`). |
| `-m, --model` | string(s) | `gpt-4.1` | Model(s) to evaluate, space-separated. Multiple models produce a benchmark leaderboard. |
| `-p, --provider` | string | `openrouter` | LLM provider: `openrouter` or `openai`. |
| `-o, --output-dir` | string | `./out` | Where results are written. |
| `-n, --parallel` | integer | — | Test cases to evaluate concurrently per model. |
| `--max-parallel` | integer | `2` | Models to evaluate concurrently (env `CALIBRATE_LLM_MAX_PARALLEL`). |
| `--overwrite` | flag | off | Ignore a prior `results.json` and start a clean run instead of resuming. |
| `-d, --debug` | flag | off | Evaluate only the first N test cases. |
| `-dc, --debug_count` | integer | `5` | Number of test cases in debug mode. |
| `--verify` | flag | off | Verify an external agent connection and exit (needs `--agent-url`). |
| `--agent-url` | string | — | External agent endpoint URL (with `--verify`). |
| `--agent-headers` | string | — | HTTP headers for the agent as a JSON string. |
| `--skip-verify` | flag | off | Skip the agent connection check before running (agent-connection configs). |
| `--eval-only` | flag | off | Skip inference; judge a `{test_case, output}` dataset (needs `--config` and `--dataset`). |
