...
<AccordionGroup>
  ...
  <Accordion title="Agent connection (Chat agent) example">
    ...
    Refer to [Agent Connections → Chat agent](/local/agent-connections/chat) for the full reference.
  ...
  <Accordion title="Agent connection (Outbound voice agent) example">
    See the [full file on GitHub](https://github.com/ARTPARK-SAHAI-ORG/calibrate/tree/main/examples/agent/simulation/sample_voice_agent_connection.json).

    Refer to [Agent Connections → Outbound voice agent](/local/agent-connections/outbound-voice) for the full reference.
...
Pass `--type text` or `--type voice` and a config file with `-c`. Set the API
keys for the selected provider in your environment (or a `.env` file in the
directory you run the command from). For text simulations against a Calibrate
agent, `-m`/`-p` set the model and provider used for both the agent and the user
simulator; for agent connections and voice, only `OPENAI_API_KEY` is required
(used by the user simulator and LLM judge). Use `-n` to control parallelism
(default: `1`).
...
```bash
calibrate-agent simulations --type text -c config.json -o ./out
```
...
```bash
calibrate-agent simulations --type text -c config.json -o ./out --skip-verify
```
...
## Evaluate existing transcripts

Already have conversation transcripts? Skip running the simulation and score them directly with `--eval-only` (text simulations only):

```bash
calibrate-agent simulations --type text -c config.json --eval-only --dataset transcripts.json -o ./out
```

`--dataset` is a JSON list of `{conversation_history, name?}` items. To only check that an external agent endpoint is reachable, use `--verify --agent-url <url>`.

## Options

| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `-t, --type` | string | — | Simulation type: `text` or `voice`. |
| `-c, --config` | string | — | Simulation config JSON. |
| `-o, --output-dir` | string | `./out` | Where results are written. |
| `-m, --model` | string | — | Model for text simulations (drives both the agent and the user simulator). |
| `-p, --provider` | string | `openrouter` | LLM provider for text simulations: `openrouter` or `openai`. |
| `-n, --parallel` | integer | `1` | Simulations to run concurrently (env `CALIBRATE_SIMULATION_PARALLEL`). |
...
| `--skip-verify` | flag | off | Skip the agent connection check before running. |
| `--eval-only` | flag | off | Skip the simulation; judge pre-existing transcripts (text only; needs `--dataset`). |
| `--dataset` | string | — | Path to the `--eval-only` transcripts JSON. |
