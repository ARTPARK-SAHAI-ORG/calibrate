...
description: "Run fully automated text-only simulations between two LLMs."
---

Run fully automated text-only conversations between two LLMs—the "agent" plus multiple users having specific personas to mimic specific scenarios.
...
# Define your tools (see Tool Types section below for webhook tools)
tools = [
    {
...
Use `"type": "structured_output"` (or `"client"`) for tools that extract structured data. Parameters are defined at the top level.
...
Use `"type": "webhook"` for tools that call external APIs. Webhook tools make actual HTTP calls during simulations.
