...
Navigate to **Agents** → **Add agent** → Name it "Customer Support Bot".
...
Go to **Agents** → "Customer Support Bot" → **Tools** tab → **Attach tool** → Select `create_refund_request`.
...
Click **Simulations** → **Add simulation**.
...
This creates 4 test conversations (2 personas × 2 scenarios):
...
1. Go to **Simulations** → Click on "Customer Support Regression Test"
...
- 🔄 **Spinner**: Conversation in progress
...
| Column                 | Description                       |
| ---------------------- | --------------------------------- |
| **Persona × Scenario** | The test case combination         |
...
| Persona × Scenario   | tool_calls_accuracy | customer_satisfaction |
| -------------------- | ------------------- | --------------------- |
| Patient × Refund     | ✅ Pass             | ✅ Pass               |
| Patient × Inquiry    | ✅ Pass             | ✅ Pass               |
| Frustrated × Refund  | ✅ Pass             | ❌ Fail               |
| Frustrated × Inquiry | ✅ Pass             | ✅ Pass               |

<Tip>Hover over any metric result to see the evaluation reasoning.</Tip>
...
Click the **play button** (▶) on any row to view the full conversation:
...
Voice simulations run complete conversations using the full pipeline: STT → LLM → TTS. This tests your agent exactly as it will perform in production.
...
<Note>
  Voice simulations take longer (1-3 minutes per conversation) because they
  process audio through STT and TTS providers.
</Note>
...
| Column                 | Description                          |
| ---------------------- | ------------------------------------ |
| **Persona × Scenario** | The test case                        |
...
| -------------------- | -------- | -------- | -------- | ----- |
| Patient × Refund     | 145ms    | 380ms    | 120ms    | 1.2s  |
| Patient × Inquiry    | 152ms    | 420ms    | 115ms    | 1.3s  |
| Frustrated × Refund  | 138ms    | 395ms    | 125ms    | 1.1s  |
| Frustrated × Inquiry | 160ms    | 410ms    | 118ms    | 1.4s  |
...
Click the **play button** (▶) on any row to open the transcript with audio:
...
```
▶ [User - Patient Customer] (0:00 - 0:04)
   "Hi, I received a defective product and would like a refund."
...
| **Latency metrics**      | ❌ No           | ✅ Yes (STT, LLM, TTS timing)     |
| **Audio playback**       | ❌ No           | ✅ Yes                            |
| **Interruption testing** | ❌ No           | ✅ Yes                            |

<Tip>
  Start with text simulations for rapid iteration, then validate with voice
...
<AccordionGroup>
  <Accordion title="Run text simulations first">
    Fix logic issues with fast text simulations before running expensive voice
    simulations.
  </Accordion>
  ...
  <Accordion title="Listen to voice outputs">
    Audio often reveals issues metrics miss: unnatural pauses, pronunciation
    problems.
  </Accordion>
  ...
  <Accordion title="Test with challenging personas">
    If a frustrated persona fails, fix that first - easier personas will likely
    pass.
  </Accordion>
</AccordionGroup>
...
4. ✅ Created 2 personas (Patient and Frustrated)
5. ✅ Created 2 scenarios (Refund and Inquiry)
6. ✅ Created 2 metrics (Tool Accuracy and Satisfaction)
7. ✅ Created a simulation with all components
8. ✅ Ran text simulation and analyzed metrics
9. ✅ Ran voice simulation with latency metrics and audio
...
<CardGroup cols={2}>
  <Card title="Personas" icon="microphone" href="/core-concepts/personas">
    Understand the best practices for creating personas
  </Card>
  <Card title="Scenarios" icon="microphone" href="/core-concepts/scenarios">
    Understand the best practices for creating scenarios
  </Card>
</CardGroup>
