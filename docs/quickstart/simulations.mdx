...
<AccordionGroup>
  ...
  <Accordion title="Connect your existing agent" icon="link">
    ...
    <Note>
      HTTP agent connections support **text simulations** only. For **voice simulations** against an existing agent, use a WebSocket `agent_url` — see [Voice Agent Connections](/core-concepts/voice-agent-connections). To get per-component STT/TTS latency metrics, use an agent built within Calibrate with STT and TTS providers configured.
    </Note>
