...
You need to configure the `tools` that your agent uses in production in the config file for running [evaluating your LLM](/local/text-to-text) and [running simulations](/local/simulations) via CLI. For a primer on tools, refer to [Tools](/core-concepts/tools).
