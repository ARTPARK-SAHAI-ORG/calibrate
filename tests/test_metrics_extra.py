    ...
    async def test_stt_llm_judge_default_evaluator_no_langfuse(self):
        ...
        with (
            patch.object(
                ...
                "text_judge",
                AsyncMock(
                    return_value={"semantic_match": {"reasoning": "r", "match": True}}
                ),
            ),
            ...
            inner = (
                M.stt_llm_judge.__wrapped__
                if hasattr(M.stt_llm_judge, "__wrapped__")
                else M.stt_llm_judge
            )
...
class TestTTSLlmJudge(unittest.IsolatedAsyncioTestCase):
    async def test_tts_llm_judge_default_evaluator(self):
        from calibrate_agent.tts import metrics as M

        with patch.object(
            M,
            "audio_judge",
            AsyncMock(
                return_value={"pronunciation": {"reasoning": "r", "match": True}}
            ),
        ):
            inner = (
                M.tts_llm_judge.__wrapped__
                if hasattr(M.tts_llm_judge, "__wrapped__")
                else M.tts_llm_judge
            )
...
class TestLlmMetrics(unittest.IsolatedAsyncioTestCase):
    async def test_test_response_llm_judge(self):
        from calibrate_agent.llm import metrics as M

        with patch.object(
            M,
            "text_judge",
            AsyncMock(return_value={"correctness": {"reasoning": "r", "match": True}}),
        ) as mock_tj:
            result = await M.test_response_llm_judge(
                ...
                response="Hi there",
                evaluators=[
                    {"name": "correctness", "system_prompt": "...", "judge_model": "x"}
                ],
            )
    ...
    async def test_evaluate_simulation_delegates(self):
        ...
        with patch.object(
            M, "simulation_judge", AsyncMock(return_value={"x": {"match": True}})
        ) as mock_sj:
