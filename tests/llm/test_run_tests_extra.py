...
class TestAnyWildcardParam(unittest.TestCase):
    ...
    def test_wildcard_contributes_no_param_record(self):
        ...
        with patch(
            "calibrate_agent.llm.run_tests.text_judge", side_effect=fake_text_judge
        ):
            result = asyncio.run(
                evaluate_tool_calls(
                    [
...
class TestNestedToolCallCriteria(unittest.TestCase):
    ...
    def test_multi_call_pass_prefixes_tool_in_reasoning(self):
        ...
        with patch(
            "calibrate_agent.llm.run_tests.text_judge",
            side_effect=self._judge(True, "ok"),
        ):
            result = asyncio.run(
                evaluate_tool_calls(
                    [
...
class TestEvaluateResponse(unittest.IsolatedAsyncioTestCase):
    async def test_evaluate_response_passing(self):
        ...
        with patch(
            "calibrate_agent.llm.run_tests.test_response_llm_judge",
    ...
    async def test_eval_only_debug_truncates_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch(
                    "calibrate_agent.llm.run_tests.run_eval_only_tests",
                    side_effect=fake_eval,
                ),
                patch(
                    "calibrate_agent.llm.run_tests.validate_llm_eval_only_dataset",
                    return_value=(True, ""),
                ),
            ):
    ...
    async def test_debug_truncates_config_test_cases(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch(
                    "calibrate_agent.llm.run_tests.run_model_tests",
